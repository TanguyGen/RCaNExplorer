...
PredationSeries<-function(Data,
  ...
  Filtered_data<-Data%>%
    filter(str_detect(Var, paste0("^(", paste(param, collapse = "|"), ")_")))%>%
    mutate(target=word(Var, 2, sep = "_"),
           series=word(Var, 1, sep = "_"))
  ...
  wrap_plots(listplot, ncol = 1) + 
    plot_annotation(title = paste("Predation on", paste(Names, collapse = ", ")),
